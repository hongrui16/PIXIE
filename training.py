...
from pixielib.utils.image_process import crop_resize_image_batch



class Worker:
    def __init__(self, debug = False, args = None) -> None:        
    ...
    def train_epoch(self, epoch):
        ...
        for idx, batch in enumerate(tbar):
            ...
            cropped_images_hd, scales_x, scales_y, left_pad, top_pad = crop_resize_image_batch(image, bboxes, self.scale, self.hd_size)
            ...
            scales_x *= self.input_size / self.hd_size
            scales_y *= self.input_size / self.hd_size
            scales_x_expanded = scales_x.view(-1, 1)  # [bs, 1]
            scales_y_expanded = scales_y.view(-1, 1)
            gt_pose_keypoints_2d[:, :, 0] *= scales_x_expanded
            gt_pose_keypoints_2d[:, :, 1] *= scales_y_expanded
            gt_hand_left_keypoints_2d[:, :, 0] *= scales_x_expanded
            gt_hand_left_keypoints_2d[:, :, 1] *= scales_y_expanded
            gt_hand_right_keypoints_2d[:, :, 0] *= scales_x_expanded
            gt_hand_right_keypoints_2d[:, :, 1] *= scales_y_expanded
            gt_face_keypoints_2d[:, :, 0] *= scales_x_expanded
            gt_face_keypoints_2d[:, :, 1] *= scales_y_expanded
